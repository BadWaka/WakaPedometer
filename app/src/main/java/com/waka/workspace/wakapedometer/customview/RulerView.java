...
//        mBgPaint.setColor(Color.argb(255, 224, 95, 23));
    ...
    private void initValue() {
        mIndicatorHalfWidth = convertDpToPixel(mContext, 9);
        mPartitionWidth = convertDpToPixel(mContext, 140.3f);
        mHighLineWidth = convertDpToPixel(mContext, 1.67f);
        mShortLineWidth = convertDpToPixel(mContext, 1.67f);
        mLineTopMargin = convertDpToPixel(mContext, 0.33f);
        mHighLineHeight = convertDpToPixel(mContext, 15.3f);
        mShortLineHeight = convertDpToPixel(mContext, 7.3f);
        mIndicatorTextTopMargin = convertDpToPixel(mContext, 15f);
            ...
            if (val >= mStartValue && val <= mEndValue) {
                ...
                if (startx > 0 && startx < mWidth) {
                    ...
                    canvas.drawText(val + "", mWidth / 2 + mOffset + i * mPartitionWidth - mIndicatorTxtPaint.measureText(val + "") / 2,
                            0 + mLineTopMargin + mHighLineHeight + mIndicatorTextTopMargin + calcTextHeight(mIndicatorTxtPaint, val + ""), mIndicatorTxtPaint);
                }
    ...
    public void setPartitionWidthInDP(float mPartitionWidth) {
        this.mPartitionWidth = convertDpToPixel(mContext, mPartitionWidth);
        recaculate();
        invalidate();
    }
